		...
		<sec>
			...
			<xsl:for-each select="node()[@object_id='41'][normalize-space(.)!='' or count(.//node()[@citation_id!=''])!=0]"><!-- Treatments -->
				<xsl:variable name="status-animalia">
					<xsl:call-template name="taxon-status-new"><xsl:with-param name="kingdom" select="'animalia'"/></xsl:call-template>
				</xsl:variable>
				<xsl:variable name="status-plantae">
					<xsl:call-template name="taxon-status-new"><xsl:with-param name="kingdom" select="'plantae'"/></xsl:call-template>
				</xsl:variable>
